            ...
            if @user
              if @user.authtoken_expiry > Time.now
                ...
                if authenticate_user && !authenticate_user.nil?
                  ...
                  new_password_digest = BCrypt::Engine.has_secret(new_password, new_password_salt)
                  
                  @user.update_attributes(password: new_password, api_authtoken: auth_token, authtoken_expiry: auth_expiry, password_salt: new_password_saly, password_hash: new_password_digest)
                  render json: @user.to_json, status: 200
                else
                  e = Error.new(status: 401, message: "Wrong Password")
                  render json: e.to_json, status: 401
